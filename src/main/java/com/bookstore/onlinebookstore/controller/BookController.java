...
public class BookController {
	...
	/**
	 * GET /api/books/category/{category}/sort/price - Get books by category sorted by price ascending
	 * Supports "all-genre" to return all books sorted by price.
	 * @param category - Book category
	 * @return JSON with sorted books
	 */
	@GetMapping("/category/{category}/sort/price")
	public Map<String, Object> getBooksByCategorySortedByPrice(@PathVariable String category) {
		Map<String, Object> response = new HashMap<>();
		try {
			Object[] books = bookService.getBooksByCategorySortedByPrice(category);
			response.put("status", "success");
			response.put("message", "Books in category '" + category + "' sorted by price retrieved successfully");
			response.put("count", books.length);
			response.put("sortBy", "price");
			response.put("books", books);
			return response;
		} catch (Exception e) {
			response.put("status", "error");
	...
	/**
	 * GET /api/books/category/{category}/sort/name - Get books by category sorted by name ascending
	 * Supports "all-genre" to return all books sorted by name.
	 * @param category - Book category
	 * @return JSON with sorted books
	 */
	@GetMapping("/category/{category}/sort/name")
	public Map<String, Object> getBooksByCategorySortedByName(@PathVariable String category) {
		Map<String, Object> response = new HashMap<>();
		try {
			Object[] books = bookService.getBooksByCategorySortedByName(category);
			response.put("status", "success");
			response.put("message", "Books in category '" + category + "' sorted by name retrieved successfully");
			response.put("count", books.length);
			response.put("sortBy", "name");
			response.put("books", books);
			return response;
		} catch (Exception e) {
			response.put("status", "error");
	 ...
	 * @return JSON with book or error
	 */
	@GetMapping("/{bookId}")
