(function () {
  ...
  async function loadBook() {
    ...
    // Wire up Add to Cart button now that we have book data
    const addToCartBtn = document.getElementById('addToCartBtn');
    if (addToCartBtn) {
      addToCartBtn.addEventListener('click', async () => {
        if (!window.BookstoreCart || !window.BookstoreCart.requireLoginForCart()) return;

        const originalText = addToCartBtn.innerHTML;
        addToCartBtn.disabled = true;
        addToCartBtn.innerHTML = '<i class="icon icon-shopping-cart me-2"></i>Adding...';
        ...
        try {
          await window.BookstoreCart.addToCart({
            ...
            imageUrl: book.imageUrl || 'images/product-placeholder.jpg',
            quantity: 1
          });
          addToCartBtn.innerHTML = '<i class="icon icon-shopping-cart me-2"></i>Added!';
          setTimeout(() => {
